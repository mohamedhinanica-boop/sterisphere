-- RC9 Slice 3C1A - workstation atomic activation and execution-item completion persistence primitives.
-- Forward-only migration draft. Apply manually in the Supabase SQL editor after review.

-- Atomic workstation shell activation boundary.
-- Apply manually in the Supabase SQL editor after review.
-- This function activates only the selected workstation shell referenced by the currently running workstation_shell item.
...
create or replace function public.activate_deployment_workstation_shell(
  p_clinic_id uuid,
  ...
  p_expected_workstation_key text,
...
returns table (
  ...
  workstation_state_before jsonb,
  workstation_state_after jsonb,
...
comment on function public.activate_deployment_workstation_shell(
  ...
  'Atomically activates only the selected setup-draft workstation shell for the running workstation_shell execution item. It does not mutate sessions, execution items, dependencies, leases, tokens, clinics, or other workstation rows.';

revoke execute on function public.activate_deployment_workstation_shell(
  uuid, text, uuid, text, text, text, timestamptz, uuid, text, text,
  integer, text, text, text, timestamptz, integer, uuid, text, jsonb, jsonb, timestamptz
) from public;
...
grant execute on function public.activate_deployment_workstation_shell(
  uuid, text, uuid, text, text, text, timestamptz, uuid, text, text,
  integer, text, text, text, timestamptz, integer, uuid, text, jsonb, jsonb, timestamptz
) to service_role;

-- Atomic workstation-shell activation execution-item completion boundary.
-- Apply manually with CREATE OR REPLACE FUNCTION only when promoting this slice.
-- This function completes only the selected running workstation_shell activate item after the workstation row is active.

create or replace function public.complete_deployment_workstation_shell_execution_item(
  p_clinic_id uuid,
  p_deployment_run_key text,
  p_session_id uuid,
  p_execution_key text,
  ...
  p_expected_entity_type text,
  p_expected_entity_id text,
  p_expected_deployment_workstation_key text,
  p_expected_action text,
  p_expected_item_started_at timestamptz,
  p_expected_attempt_count integer,
  p_workstation_id uuid,
  p_expected_workstation_state jsonb,
...
revoke all on function public.complete_deployment_workstation_shell_execution_item(
...
grant execute on function public.complete_deployment_workstation_shell_execution_item(
  uuid, text, uuid, text, text, text, timestamptz, uuid, text, text, integer, text, text, text, text, timestamptz, integer, uuid, jsonb, jsonb, timestamptz
) to service_role;
