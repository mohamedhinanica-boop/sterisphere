...
## RC8 Slice 11E - Runtime Post-Provider Next Item Start Sequence
...
10. Atomic provider-shell execution-item completion
11. Post-provider atomic dependency progression
12. Post-provider atomic next execution-item start
...
Setup Complete consumes the already-running first `workstation_shell:activate` item directly from the sterilizer sequence result. Prepared execution evidence supplies the authoritative workstation count and UUID/key identities. The Generic Entity Sequence Driver owns deterministic serial iteration, duplicate-ID and exactly-once controls, identity/ownership/lease validation, bound enforcement, aggregate counters, and the first non-matching handoff.

Each matching item follows `workstation activation -> same-item completion -> dependency progression -> next-item start`. Additional workstation items continue serially. After the authoritative bound, the first `hardware_shell:activate` item may be started and returned without dispatch or activation. No duplicate workstation loop, rollback, session completion, deployment finalization, retry, polling, or background execution is added.
