-- RC9 Slice 3C1A read-only workstation activation and completion verification.

-- RC9 Slice 3C1A workstation shell activation preflight.
...
with required_tables as (
  ...
  union all select 'required_table_clinical_workstations', to_regclass('public.clinical_workstations') is not null
), required_workstation_columns as (
  select 'required_workstation_columns' as check_name,
    count(*) filter (where column_name in ('id','clinic_id','deployment_workstation_key','provisioning_source','provisioning_status','active','updated_at')) = 7 as passed
  from information_schema.columns
  where table_schema = 'public'
    and table_name = 'clinical_workstations'
), function_contract as (
  select 'exact_rpc_signature_exists' as check_name,
    to_regprocedure('public.activate_deployment_workstation_shell(uuid,text,uuid,text,text,text,timestamptz,uuid,text,text,integer,text,text,text,timestamptz,integer,uuid,text,jsonb,jsonb,timestamptz)') is not null as passed
  ...
  where n.nspname = 'public'
    and p.proname = 'activate_deployment_workstation_shell'
), privileges as (
  select 'service_role_only_execute' as check_name,
    has_function_privilege('service_role', 'public.activate_deployment_workstation_shell(uuid,text,uuid,text,text,text,timestamptz,uuid,text,text,integer,text,text,text,timestamptz,integer,uuid,text,jsonb,jsonb,timestamptz)', 'execute')
    and not has_function_privilege('anon', 'public.activate_deployment_workstation_shell(uuid,text,uuid,text,text,text,timestamptz,uuid,text,text,integer,text,text,text,timestamptz,integer,uuid,text,jsonb,jsonb,timestamptz)', 'execute')
    and not has_function_privilege('authenticated', 'public.activate_deployment_workstation_shell(uuid,text,uuid,text,text,text,timestamptz,uuid,text,text,integer,text,text,text,timestamptz,integer,uuid,text,jsonb,jsonb,timestamptz)', 'execute') as passed
), workstation_duplicates as (
  select 'no_duplicate_workstation_deployment_identity' as check_name,
    not exists (
      select 1
      from public.clinical_workstations workstation_row
      where workstation_row.clinic_id is not null
        and workstation_row.deployment_workstation_key is not null
      group by workstation_row.clinic_id, workstation_row.deployment_workstation_key
      having count(*) > 1
    ) as passed
), workstation_lifecycle_assumptions as (
  select 'workstation_lifecycle_status_assumptions' as check_name,
    not exists (
      select 1
      from public.clinical_workstations workstation_row
      where workstation_row.deployment_workstation_key is not null
        and workstation_row.provisioning_source = 'setup_draft'
        and workstation_row.provisioning_status not in ('planned', 'active')
    ) as passed
), active_incompatible as (
  select 'no_active_incompatible_workstation_shells' as check_name,
    not exists (
      select 1
      from public.clinical_workstations workstation_row
      where workstation_row.deployment_workstation_key is not null
        and workstation_row.active = true
        and (workstation_row.provisioning_source is distinct from 'setup_draft' or workstation_row.provisioning_status is distinct from 'active')
    ) as passed
), running_item_integrity as (
  select 'running_workstation_shell_item_integrity_snapshot' as check_name,
    true as passed,
    jsonb_build_object(
      'runningWorkstationItems', count(*) filter (where item_row.execution_status = 'running' and item_row.entity_type = 'workstation_shell'),
  ...
  select lower(regexp_replace(pg_get_functiondef('public.activate_deployment_workstation_shell(uuid,text,uuid,text,text,text,timestamptz,uuid,text,text,integer,text,text,text,timestamptz,integer,uuid,text,jsonb,jsonb,timestamptz)'::regprocedure), '\s+', ' ', 'g')) as body
), source_diagnostics as (
  select
    body ~ '(^|[^a-z_])update\s+public\.clinical_workstations\s+update_workstation([^a-z_]|$)' as updates_clinical_workstations,
    regexp_count(body, '(^|[^a-z_])update\s+public\.clinical_workstations\s+update_workstation([^a-z_]|$)') = 1 as updates_selected_workstation_once,
    body ~ 'where\s+update_workstation\.id\s*=\s*v_workstation\.id' as constrains_workstation_id,
    body ~ 'and\s+update_workstation\.clinic_id\s*=\s*p_clinic_id' as constrains_clinic_id,
    body ~ 'and\s+update_workstation\.deployment_workstation_key\s*=\s*p_expected_workstation_key' as constrains_workstation_key,
    ...
    body ~ '''entityidmatchesworkstationid'',\s*v_item\.entity_id::text\s+is\s+not\s+distinct\s+from\s+p_workstation_id::text' as reports_entity_id_workstation_id_diagnostic,
    body !~ 'p_expected_entity_id\s+is\s+distinct\s+from\s+p_expected_workstation_key' as does_not_compare_entity_id_to_workstation_key,
    body !~ 'v_item\.entity_id::text\s+is\s+distinct\s+from\s*p_expected_workstation_key' as does_not_compare_item_entity_id_to_workstation_key,
    body ~ 'v_item\.deployment_key\s+is\s+distinct\s+from\s+p_expected_workstation_key' as compares_item_deployment_key_to_workstation_key,
    body ~ 'v_item_transition_state\s*:=\s*jsonb_build_object\s*\([^;]*''deploymentworkstationkey''[^;]*''provisioningsource''[^;]*''provisioningstatus''[^;]*''active''' as projects_authoritative_item_transition_state,
  ...
  select 'function_updates_only_clinical_workstations' as check_name,
    updates_clinical_workstations
    and updates_selected_workstation_once
    and constrains_workstation_id
    and constrains_clinic_id
    and constrains_workstation_key
    ...
    jsonb_build_object(
      'updates_clinical_workstations', updates_clinical_workstations,
      'updates_selected_workstation_once', updates_selected_workstation_once,
      'constrains_workstation_id', constrains_workstation_id,
      'constrains_clinic_id', constrains_clinic_id,
      'constrains_workstation_key', constrains_workstation_key,
      ...
      'does_not_compare_entity_id_to_workstation_key', does_not_compare_entity_id_to_workstation_key,
      'does_not_compare_item_entity_id_to_workstation_key', does_not_compare_item_entity_id_to_workstation_key,
      'compares_item_deployment_key_to_workstation_key', compares_item_deployment_key_to_workstation_key
    ) as details
  from source_diagnostics
  union all
  select 'function_preserves_workstation_uuid_key_identity_contract',
    compares_item_entity_id_to_expected_entity_id
    and reports_entity_id_workstation_id_diagnostic
    and does_not_compare_entity_id_to_workstation_key
    and does_not_compare_item_entity_id_to_workstation_key
    and compares_item_deployment_key_to_workstation_key,
    jsonb_build_object(
      'compares_item_entity_id_to_expected_entity_id', compares_item_entity_id_to_expected_entity_id,
      'reports_entity_id_workstation_id_diagnostic', reports_entity_id_workstation_id_diagnostic,
      'does_not_compare_entity_id_to_workstation_key', does_not_compare_entity_id_to_workstation_key,
      'does_not_compare_item_entity_id_to_workstation_key', does_not_compare_item_entity_id_to_workstation_key,
      'compares_item_deployment_key_to_workstation_key', compares_item_deployment_key_to_workstation_key
    )
  from source_diagnostics
  union all
  select 'function_compares_authoritative_workstation_transition_state',
  ...
  from source_diagnostics
  union all
  select 'function_updates_selected_workstation_once',
    updates_selected_workstation_once
    and constrains_workstation_id
    and constrains_clinic_id
    and constrains_workstation_key,
    jsonb_build_object(
...
union all select check_name, passed, null::jsonb from required_workstation_columns
...
union all select check_name, passed, null::jsonb from workstation_duplicates
union all select check_name, passed, null::jsonb from workstation_lifecycle_assumptions
...
-- RC8 Slice 11B read-only preflight for workstation-shell execution-item completion.
...
with required_tables as (
  ...
  union all select 'required_table_clinical_workstations', to_regclass('public.clinical_workstations') is not null, null::jsonb
), required_columns as (
  ...
  select 'exact_rpc_signature_exists' as check_name,
    to_regprocedure('public.complete_deployment_workstation_shell_execution_item(uuid,text,uuid,text,text,text,timestamptz,uuid,text,text,integer,text,text,text,text,timestamptz,integer,uuid,jsonb,jsonb,timestamptz)') is not null as passed,
    null::jsonb as details
), function_security as (
  select 'function_security_definer_search_path_fixed' as check_name,
    coalesce(p.prosecdef, false)
  ...
  where n.nspname = 'public' and p.proname = 'complete_deployment_workstation_shell_execution_item'
), privileges as (
  select 'service_role_only_execute' as check_name,
    has_function_privilege('service_role', 'public.complete_deployment_workstation_shell_execution_item(uuid,text,uuid,text,text,text,timestamptz,uuid,text,text,integer,text,text,text,text,timestamptz,integer,uuid,jsonb,jsonb,timestamptz)', 'execute')
    and not has_function_privilege('anon', 'public.complete_deployment_workstation_shell_execution_item(uuid,text,uuid,text,text,text,timestamptz,uuid,text,text,integer,text,text,text,text,timestamptz,integer,uuid,jsonb,jsonb,timestamptz)', 'execute')
    and not has_function_privilege('authenticated', 'public.complete_deployment_workstation_shell_execution_item(uuid,text,uuid,text,text,text,timestamptz,uuid,text,text,integer,text,text,text,text,timestamptz,integer,uuid,jsonb,jsonb,timestamptz)', 'execute') as passed,
    null::jsonb as details
), duplicate_checks as (
  select 'no_duplicate_item_or_workstation_identities' as check_name,
    not exists (
    ...
    and not exists (
      select 1 from public.clinical_workstations workstation_row where workstation_row.clinic_id is not null and workstation_row.deployment_workstation_key is not null group by workstation_row.clinic_id, workstation_row.deployment_workstation_key having count(*) > 1
    ) as passed,
    null::jsonb as details
), timestamp_checks as (
  ...
  select lower(regexp_replace(pg_get_functiondef('public.complete_deployment_workstation_shell_execution_item(uuid,text,uuid,text,text,text,timestamptz,uuid,text,text,integer,text,text,text,text,timestamptz,integer,uuid,jsonb,jsonb,timestamptz)'::regprocedure), '\s+', ' ', 'g')) as body
), update_source as (
  select
    body,
  ...
  select
    ...
    body !~ 'update\s+public\.clinical_workstations' as does_not_update_clinical_workstations,
    ...
    body !~ '(^|[^a-z_])insert\s+into([^a-z_]|$)' as does_not_insert,
    body !~ '(^|[^a-z_])delete\s+from([^a-z_]|$)' as does_not_delete,
    body ~ 'v_item\.entity_id\s+is\s+distinct\s+from\s+p_expected_entity_id' as compares_workstation_uuid_as_text,
    body ~ 'v_workstation\.id\s+is\s+distinct\s+from\s+p_workstation_id' as compares_workstation_uuid_to_workstation_id,
    body ~ 'v_workstation\.deployment_workstation_key\s+is\s+distinct\s+from\s+p_expected_deployment_workstation_key' as compares_workstation_key_to_key,
    body !~ 'p_workstation_id(::text)?\s*=\s*p_expected_deployment_workstation_key' as does_not_conflate_uuid_and_key
  from update_parts
), source_checks as (
  select 'function_mutates_only_selected_execution_item' as check_name,
    updates_execution_items and updates_selected_item_alias and updates_only_one_public_table and constrains_item_id and constrains_session_id and constrains_execution_item_key and constrains_plan_item_key and writes_succeeded_status and writes_completed_at and does_not_write_started_at and does_not_increment_attempt_count and does_not_update_sessions and does_not_update_clinical_workstations and does_not_update_clinics and does_not_write_lease and does_not_write_token and does_not_progress_dependencies and does_not_start_another_item and does_not_insert and does_not_delete as passed,
    jsonb_build_object(
      ...
      'does_not_update_clinical_workstations', does_not_update_clinical_workstations,
      'does_not_update_clinics', does_not_update_clinics,
      'does_not_write_lease', does_not_write_lease,
      'does_not_write_token', does_not_write_token,
  ...
  from source_diagnostics
  union all
  select 'function_preserves_workstation_uuid_key_distinction',
    compares_workstation_uuid_as_text and compares_workstation_uuid_to_workstation_id and compares_workstation_key_to_key and does_not_conflate_uuid_and_key,
    jsonb_build_object(
      'compares_workstation_uuid_as_text', compares_workstation_uuid_as_text,
      'compares_workstation_uuid_to_workstation_id', compares_workstation_uuid_to_workstation_id,
      'compares_workstation_key_to_key', compares_workstation_key_to_key,
