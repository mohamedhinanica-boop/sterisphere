...
The atomic RPC locks the execution session and selected item, rechecks owner, ownership token, expected lease, running session state, item identity, sequence, entity/action, started timestamp, attempt count, completion absence, rollback absence, error absence, item-count integrity, and duplicate-free item identities. The only successful mutation is `deployment_activation_execution_items.execution_status = 'succeeded'` plus an authoritative database mutation-time `completed_at = clock_timestamp()` value for that one item. The caller timestamp remains a compatibility/assessment input and cannot determine the stored completion time.
...
## RC9 Slice 2D - Provider runtime composition boundary

The already-running `provider_shell:activate` item is now the second production consumer of the server-only generic execution-step orchestrator. The setup provider branch supplies one durable running provider item to `executeDeploymentExecutionStepForServer`, which delegates in order to the existing provider activation, provider item-completion, dependency-progression, and at-most-one next-item-start boundaries, returns token-safe structured evidence, and stops.

Provider UUID `entityId` remains distinct from the deterministic provider `deploymentKey`; `deploymentRunKey`, clinic, claim, lease, execution, plan, item, lifecycle, state, dependency, and rollback-instruction evidence are preserved. The clinic generic branch is unchanged. Existing outer provider ordering is unchanged, and there is no duplicated provider path, fallback, retry, recursion, session completion, deployment finalization, rollback execution, worker, queue, polling, or background execution.

## RC9 Slice 2E - Bounded provider-sequence driver
...
RC9 Slice 3B1B adds the server-only TypeScript activation and completion repositories, Supabase RPC adapters, validation services, composition boundaries, and an isolated `sterilizer_shell:activate` one-step handler. The handler performs activation followed by execution-item completion only.

RC9 Slice 3B2 registers that handler through the authoritative production registry and consumes the already-running provider-to-sterilizer handoff with the Generic Entity Sequence Driver. The authoritative bound is the prepared `sterilizer_shell:activate` item count. Each sterilizer is activated and completed exactly once through the atomic RPC-backed boundaries before generic dependency progression and next-item start. The first workstation item may be started and returned as the non-matching handoff, but it is not dispatched or activated. Workstation migration remains RC9 Slice 3C; rollback, finalization, heartbeat renewal, and recovery remain unavailable.

## RC9 Slice 3C1A — Workstation atomic persistence primitives

Two service-role-only, `SECURITY DEFINER` RPC foundations define the future workstation execution boundary: `activate_deployment_workstation_shell(...)` atomically validates the owned running item and transitions only its clinic-scoped workstation from setup-draft `planned`/inactive to `active`/active; `complete_deployment_workstation_shell_execution_item(...)` proves that durable state and transitions only the same running execution item to `succeeded`. Both functions use a fixed `pg_catalog, public` search path, return structured status and issue codes without tokens, and preserve UUID, clinic, deterministic key, source, dependency, rollback, session, and finalization state.

The planner target remains transition-only: `{ provisioningStatus: "active", active: true }`. UUID, clinic, `deployment_workstation_key`, and `provisioning_source` are validated separately. Exact already-activated and already-completed evidence may be reused only while ownership, claimant, lease, execution-item identity, immutable workstation identity, and durable target state still match. This slice adds no TypeScript repository, service, handler, registry, sequence-driver, or Setup Complete runtime wiring.

## RC9 Slice 3C1B — Workstation activation TypeScript foundation

The workstation activation boundary now has server-only repository interfaces, Supabase RPC adapters, validation services, composition helpers, test repositories, and an isolated `workstation_shell:activate` one-step handler. Activation calls only `activate_deployment_workstation_shell(...)`; completion calls only `complete_deployment_workstation_shell_execution_item(...)`. The handler preserves clinic, deployment-run, execution-session, execution-item, claimant, ownership, lease, UUID, deterministic deployment-key, immutable-state, transition-target, dependency, and rollback evidence while performing only activation followed by completion.

The workstation handler is intentionally absent from the production registry and Generic Entity Sequence Driver. It performs no dependency progression, next-item start, sequence iteration, retry, rollback, session completion, or deployment finalization, so workstation activation remains unreachable until the later runtime-adapter slice.

## RC9 Slice 3C2 — Workstation generic sequence runtime

The authoritative production registry now resolves `workstation_shell:activate`. Setup Complete passes the already-running workstation handoff returned by the sterilizer sequence directly into the Generic Entity Sequence Driver; the item is not readied, selected by position, or started again. Prepared `workstation_shell:activate` execution items provide the sole authoritative bound, while the generic driver retains bounded serial ordering, duplicate-ID and exactly-once controls, identity/ownership/lease validation, aggregate counters, and first non-matching handoff.

For each workstation, the registered handler performs atomic activation followed by atomic completion of the same item, then the existing outer boundaries progress dependencies and start at most one next item. After all prepared workstations complete, the first hardware item may be started and returned as the non-matching handoff, but it is not dispatched or activated. Clinic, provider, and sterilizer semantics are unchanged; hardware activation, rollback, session completion, finalization, retry, and background execution remain unavailable.

## RC9 Slice 3D1A — Hardware atomic persistence primitives

Two service-role-only, `SECURITY DEFINER` RPC foundations define the future hardware execution boundary. `activate_deployment_hardware_shell(...)` atomically validates the owned running item and transitions only its clinic-scoped hardware shell from setup-draft `planned`/inactive to `active`/active. `complete_deployment_hardware_shell_execution_item(...)` revalidates the active durable hardware state and transitions only the same running execution item to `succeeded`. Both functions use a fixed `pg_catalog, public` search path and return structured status and issue evidence without ownership tokens.

Hardware UUID and deterministic `deployment_hardware_key` remain distinct. Immutable activation evidence includes provisioning source/status, active state, operational status, agent identity, and default/current workstation bindings; the target remains transition-only: `{ provisioningStatus: "active", active: true }`. Exact already-activated and already-completed evidence may be reused while preserving the original completion timestamp. This slice adds no repository, service, handler, registry, sequence-driver, Setup Complete, hardware-assignment, rollback, session-completion, or finalization changes.

## RC9 Slice 3D1B � Hardware TypeScript persistence foundation
