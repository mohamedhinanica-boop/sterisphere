...
const PROVIDER_ACTIVATION_SESSION_COLUMNS = [
  "id",
  "clinic_id",
  "deployment_run_key",
  "execution_key",
...
const PROVIDER_ACTIVATION_ITEM_COLUMNS = [
  "id",
  "session_id",
  "execution_item_key",
  "plan_item_key",
...
const PROVIDER_ACTIVATION_PROVIDER_COLUMNS = [
  "id",
  "clinic_id",
...
const PROVIDER_ACTIVATION_RPC_NAME = "activate_deployment_provider_shell";
const REDACTED = "[redacted]";

export type ProviderShellActivationSessionRow = {
  id: string;
  clinic_id: string;
...
export type ProviderShellActivationItemRow = {
  id: string;
  session_id: string;
...
export type ProviderShellActivationProviderRow = {
  id: string;
...
type ProviderShellActivationRpcRow = {
  status: string | null;
  clinic_id: string | null;
  ...
  deployment_provider_key: string | null;
  provider_state_before: unknown;
  provider_state_after: unknown;
...
export class DeploymentProviderShellActivationRepositoryError extends Error {
  ...
  constructor(input: {
    ...
    super(input.message);
    this.name = "DeploymentProviderShellActivationRepositoryError";
...
export class SupabaseDeploymentProviderShellActivationRepository
  implements DeploymentProviderShellActivationRepository
{
  constructor(private readonly client: SupabaseClient) {}

  async loadProviderShellActivationSnapshot(input: {
    clinicId: string;
  ...
  async activateProviderShellAtomically(
    ...
    const payload = providerShellActivationRpcPayload(command);
    const { data, error } = await this.client.rpc(PROVIDER_ACTIVATION_RPC_NAME, payload);

    if (error) {
      throw toRepositoryError(error, command.ownershipToken, "atomic_rpc");
    }

    return mapProviderShellActivationRpcResult(readSingleRpcRow(data, "atomic_rpc_response_mapping"));
  }
  ...
  private async findSession(input: {
    ...
    sessionId: string;
    executionKey: string;
  }): Promise<ProviderShellActivationSessionRow | null> {
    const { data, error } = await this.client
      .from("deployment_activation_execution_sessions")
      .select(PROVIDER_ACTIVATION_SESSION_COLUMNS)
    ...
    const rows = (data ?? []) as unknown as ProviderShellActivationSessionRow[];
    assertAtMostOne(rows, "provider shell activation session");

    return rows[0] ?? null;
  }

  private async listItems(sessionId: string): Promise<readonly ProviderShellActivationItemRow[]> {
    const { data, error } = await this.client
      .from("deployment_activation_execution_items")
      .select(PROVIDER_ACTIVATION_ITEM_COLUMNS)
    ...
    return (data ?? []) as unknown as ProviderShellActivationItemRow[];
  }

  private async listProviders(
    clinicId: string,
    deploymentProviderKey: string,
  ): Promise<readonly ProviderShellActivationProviderRow[]> {
    const { data, error } = await this.client
      .from("providers")
      .select(PROVIDER_ACTIVATION_PROVIDER_COLUMNS)
    ...
    return (data ?? []) as unknown as ProviderShellActivationProviderRow[];
  }
}

export function mapProviderShellActivationSessionRow(
  row: ProviderShellActivationSessionRow,
): DeploymentProviderShellActivationSessionSnapshot {
  return {
...
export function mapProviderShellActivationItemRow(
  row: ProviderShellActivationItemRow,
...
export function mapProviderShellActivationProviderRow(
  row: ProviderShellActivationProviderRow,
  ...
  return {
    ...
    archivedAt: null,
    deletedAt: null,
    currentState: providerState(row),
  };
}

export function aggregateProviderShellActivationRows(
  items: readonly ProviderShellActivationItemRow[],
  providers: readonly ProviderShellActivationProviderRow[],
  ...
  return {
    ...
    providerCandidateCount: providers.length,
    duplicateProviderIdentityCount: duplicateCount(providers.map((provider) => `${provider.clinic_id ?? "global"}:${provider.deployment_provider_key ?? "null"}`)),
  };
}

export function providerShellActivationRpcPayload(
  command: DeploymentProviderShellActivationAtomicCommand,
  ...
  return {
    ...
    p_expected_provider_key: command.expectedProviderKey,
...
export function mapProviderShellActivationRpcResult(
  row: ProviderShellActivationRpcRow,
): DeploymentProviderShellActivationAtomicResult {
  const status = readProviderShellActivationAtomicStatus(row.status);
  ...
  return {
    ...
    deploymentProviderKey: row.deployment_provider_key,
    providerStateBefore: readNullableRecord(row.provider_state_before),
    providerStateAfter: readNullableRecord(row.provider_state_after),
    activatedAt: row.activated_at,
    issueCode: row.issue_code,
    message: row.message ?? "Provider shell activation RPC returned no message.",
  };
}

export function readSingleRpcRow(data: unknown, layer = "rpc_response_mapping"): ProviderShellActivationRpcRow {
  const rows = Array.isArray(data) ? data : [data];

  if (rows.length !== 1) {
    throw new DeploymentProviderShellActivationRepositoryError({
      message: "Ambiguous provider shell activation RPC response.",
      layer,
    });
  }
  ...
  if (!row || typeof row !== "object" || Array.isArray(row)) {
    throw new DeploymentProviderShellActivationRepositoryError({
      message: "Malformed provider shell activation RPC response.",
      layer,
    });
  }

  return row as ProviderShellActivationRpcRow;
}

export function assertAtMostOne(rows: readonly unknown[], label: string): void {
  if (rows.length > 1) {
    throw new DeploymentProviderShellActivationRepositoryError({
      message: `Ambiguous ${label} rows prevent deterministic provider shell activation.`,
    });
  }
}

function readProviderShellActivationAtomicStatus(
  value: string | null,
): DeploymentProviderShellActivationAtomicResult["status"] {
  ...
  if (allowed.includes(value as (typeof allowed)[number])) {
    return value as DeploymentProviderShellActivationAtomicResult["status"];
  }

  throw new DeploymentProviderShellActivationRepositoryError({
    message: "Malformed provider shell activation RPC status.",
...
function getSucceededPrefix(items: readonly ProviderShellActivationItemRow[]): ProviderShellActivationItemRow[] {
  const prefix: ProviderShellActivationItemRow[] = [];
...
function hasLaterIntegrityIssue(item: ProviderShellActivationItemRow): boolean {
...
function compareRows(left: ProviderShellActivationItemRow, right: ProviderShellActivationItemRow): number {
...
function toRepositoryError(
  ...
  return new DeploymentProviderShellActivationRepositoryError({
    message: sanitizeDiagnostic(error.message || "Provider shell activation repository query failed.", sensitiveToken) ?? "Provider shell activation repository query failed.",
