...
export class DeploymentActivationPlanRepositoryError extends Error {
  readonly code: string | null;

  constructor(message: string, code: string | null = null) {
    super(message);
    this.name = "DeploymentActivationPlanRepositoryError";
    this.code = code;
  }
}

export class SupabaseDeploymentActivationPlanRepository
  implements DeploymentActivationPlanRepository
{
  ...
  async getActivationPlanSnapshot(
    ...
    return {
      ...
      existingActivationPlanKey: null,
    };
  }
...
export function externalActivationPlanEvidenceBoundary(): Pick<
  DeploymentActivationPlanSnapshot,
...
export function assertAtMostOne(
  ...
  if (rows.length > 1) {
    throw new DeploymentActivationPlanRepositoryError(
      `Duplicate ${entityName} rows prevent deterministic activation planning.`,
    );
  }
}
...
): DeploymentActivationPlanRepositoryError {
  return new DeploymentActivationPlanRepositoryError(
    error.message,
    error.code ?? null,
  );
}
