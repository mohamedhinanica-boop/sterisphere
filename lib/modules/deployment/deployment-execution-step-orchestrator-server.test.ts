...
import { createServerClinicDeploymentExecutionStepDependencies, createServerProviderDeploymentExecutionStepDependencies, executeServerProviderSequence, executeServerSterilizerSequence, executeServerWorkstationSequence, executeServerHardwareSequence, executeDeploymentExecutionStepForServer, type ServerDeploymentExecutionStepOrchestratorDependencies } from "./deployment-execution-step-orchestrator-server";
...
export async function runDeploymentExecutionStepOrchestratorServerHarness(): Promise<DeploymentExecutionStepOrchestratorServerHarnessResult> {
  const scenarios = [
    ...
    await hardwareSequenceScenario(),
    await hardwareSequenceScenario(),
  ];
...
async function hardwareSequenceScenario() {
  const prepared = [hardwarePrepared(28), hardwarePrepared(29), hardwareAssignmentPrepared(30)];
  const calls: string[] = [];
  const executeHardwareStep = async ({ current, prepared: item }: { current: any; prepared: any }) => {
    calls.push(`${current.itemId}:${current.attemptCount}:${item.executionItemKey}:${item.entityId}:${item.deploymentKey}`);
    const next = current.sequence === 28
      ? nextEvidence(29, "hardware_shell")
      : { ...nextEvidence(30, "hardware_assignment"), action: "assign" };
    return { step: stepResult(next), evidence: { hardwareActivation: { hardwareId: item.entityId, deploymentHardwareKey: item.deploymentKey } as any, itemCompletion: { itemId: current.itemId, hardwareId: item.entityId } as any, dependencyProgression: {} as any, nextItemStart: next } };
  };
  const base = { ...sequenceInput(prepared), initialNextItemStart: nextEvidence(28, "hardware_shell"), executeHardwareStep };
  const result = await executeServerHardwareSequence({} as any, base);
  const duplicate = await executeServerHardwareSequence({} as any, { ...base, executeHardwareStep: async ({ current }: any) => { const next = { ...nextEvidence(29, "hardware_shell"), itemId: current.itemId }; return { step: stepResult(next), evidence: { hardwareActivation: {} as any, itemCompletion: {} as any, dependencyProgression: {} as any, nextItemStart: next } }; } });
  const malformed = await executeServerHardwareSequence({} as any, { ...base, initialNextItemStart: { ...nextEvidence(28, "hardware_shell"), executionKey: "foreign" } });
  const early = await executeServerHardwareSequence({} as any, { ...base, executeHardwareStep: async () => { const next = { ...nextEvidence(29, "hardware_assignment"), action: "assign" }; return { step: stepResult(next), evidence: { hardwareActivation: {} as any, itemCompletion: {} as any, dependencyProgression: {} as any, nextItemStart: next } }; } });
  const exceeded = await executeServerHardwareSequence({} as any, { ...base, preparedExecutionItems: [hardwarePrepared(28), hardwareAssignmentPrepared(29)], executeHardwareStep: async () => { const next = nextEvidence(29, "hardware_shell"); return { step: stepResult(next), evidence: { hardwareActivation: {} as any, itemCompletion: {} as any, dependencyProgression: {} as any, nextItemStart: next } }; } });
  const handoff = result.nextItemStart;
  const passed = result.ok && result.hardwareItemsPlanned === 2 && result.hardwareItemsExecuted === 2 && calls.length === 2 && calls[0].startsWith("item-28:1:") && calls[1].startsWith("item-29:1:") && handoff?.entityType === "hardware_assignment" && handoff.action === "assign" && !calls.some((call) => call.includes("assignment")) && result.lastStep?.downstream.entitiesActivated === 2 && result.lastStep.downstream.itemsCompleted === 2 && result.lastStep.downstream.dependenciesProgressed === 2 && result.lastStep.downstream.itemsStarted === 2 && result.lastStep.downstream.rollbacksExecuted === 0 && result.lastStep.downstream.sessionsCompleted === 0 && result.lastStep.downstream.deploymentsFinalized === 0 && !duplicate.ok && !malformed.ok && !early.ok && !exceeded.ok;
  return scenario("bounded hardware sequence consumes workstation handoff once and stops after starting hardware assignment without dispatch", passed, JSON.stringify({ calls, result: result.message, duplicate: duplicate.message, malformed: malformed.message, early: early.message, exceeded: exceeded.message, handoff: handoff?.entityType }));
}async function hardwareSequenceScenario() {
  const prepared = [hardwarePrepared(28), hardwarePrepared(29), hardwareAssignmentPrepared(30)];
  const calls: string[] = [];
  const executeHardwareStep = async ({ current, prepared: item }: { current: any; prepared: any }) => {
    calls.push(`${current.itemId}:${current.attemptCount}:${item.executionItemKey}:${item.entityId}:${item.deploymentKey}`);
    const next = current.sequence === 28 ? nextEvidence(29, "hardware_shell") : { ...nextEvidence(30, "hardware_assignment"), action: "assign" };
    return { step: stepResult(next), evidence: { hardwareActivation: { hardwareId: item.entityId, deploymentHardwareKey: item.deploymentKey } as any, itemCompletion: { itemId: current.itemId, hardwareId: item.entityId } as any, dependencyProgression: {} as any, nextItemStart: next } };
  };
  const base = { ...sequenceInput(prepared), initialNextItemStart: nextEvidence(28, "hardware_shell"), executeHardwareStep };
  const result = await executeServerHardwareSequence({} as any, base);
...
function hardwarePrepared(sequence: number) { return { ...providerPrepared(sequence), executionItemKey: `execution-server-step:hardware-${sequence}`, planItemKey: `plan-server-step:hardware-${sequence}`, entityType: "hardware_shell", entityId: `00000000-0000-4000-8001-${String(sequence).padStart(12, "0")}`, deploymentKey: `hardware-key-${sequence}`, targetState: { provisioningStatus: "active", active: true } }; }
function hardwareAssignmentPrepared(sequence: number) { return { ...providerPrepared(sequence), executionItemKey: `execution-server-step:hardware-assignment-${sequence}`, planItemKey: `plan-server-step:hardware-assignment-${sequence}`, entityType: "hardware_assignment", entityId: `assignment-id-${sequence}`, deploymentKey: `assignment-key-${sequence}`, action: "assign" }; }
