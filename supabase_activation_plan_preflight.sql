-- RC8 Slice 1B - Controlled activation plan live preflight
-- Read-only verification for durable source rows used by
-- SupabaseDeploymentActivationPlanRepository.
--
-- Activation readiness and planned assignment resolution evidence are not
-- persisted today. This preflight verifies only durable source schema/data
-- needed to rebuild the activation-planning snapshot and detect drift.
...
select
  'activation_plan_persistence_tables' as check_name,
  table_schema,
  table_name,
  'informational_only_not_required_by_rc8_1b' as status
from information_schema.tables
where table_schema = 'public'
  and table_name in (
    'deployment_activation_plans',
    'deployment_activation_plan_items'
  )
order by table_name;
...
select 'duplicate_clinic_settings' as check_name, clinic_id, count(*) as row_count
...
select
  'planned_hardware_status_counts' as check_name,
...
select
  'durable_activation_plan_snapshot_counts' as check_name,
